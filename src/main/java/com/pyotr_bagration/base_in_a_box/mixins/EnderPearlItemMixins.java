/*
 * Repository: Base In A Box
 * Module: EnderPearlItemMixins
 * Author: The Sex Haver
 * Date: 3/16/2024
 * License: GPL V3
 * Description: Mixins that enforce the inadvisability of attempts to clip through bounds within the Box
 * Dimension through the use of Ender Pearls.
 */

package com.pyotr_bagration.base_in_a_box.mixins;

import com.pyotr_bagration.base_in_a_box.Common;
import com.pyotr_bagration.base_in_a_box.box_dimension.BoxDimension;

import net.minecraft.entity.player.PlayerEntity;
import net.minecraft.entity.projectile.thrown.EnderPearlEntity;
import net.minecraft.item.EnderPearlItem;
import net.minecraft.item.ItemStack;
import net.minecraft.sound.SoundCategory;
import net.minecraft.sound.SoundEvents;
import net.minecraft.stat.Stats;
import net.minecraft.util.Hand;
import net.minecraft.util.TypedActionResult;
import net.minecraft.world.World;

import org.spongepowered.asm.mixin.Mixin;
import org.spongepowered.asm.mixin.Overwrite;

@Mixin(EnderPearlItem.class)
public final class EnderPearlItemMixins {

    private EnderPearlItemMixins() throws InstantiationException {
        throw new InstantiationException("Class \"EnderPearlItemMixins\" cannot be instantiated.");
    }

    @Overwrite
    @SuppressWarnings("OverwriteAuthorRequired")
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ItemStack itemStack = user.getStackInHand(hand);
        world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.ENTITY_ENDER_PEARL_THROW, SoundCategory.NEUTRAL, 0.5f, 0.4f / (world.getRandom().nextFloat() * 0.4f + 0.8f));
        user.getItemCooldownManager().set((EnderPearlItem) (Object) this, 20);
        if (!world.isClient) {
            if (world.getRegistryKey() != BoxDimension.WORLD_KEY) {
                EnderPearlEntity enderPearlEntity = new EnderPearlEntity(world, user);
                enderPearlEntity.setItem(itemStack);
                enderPearlEntity.setVelocity(user, user.getPitch(), user.getYaw(), 0.0f, 1.5f, 1.0f);
                world.spawnEntity(enderPearlEntity);
            } else {
                Common.Detonate(world, user);
            }
        }
        user.incrementStat(Stats.USED.getOrCreateStat((EnderPearlItem) (Object) this));
        if (!user.getAbilities().creativeMode) {
            itemStack.decrement(1);
        }
        return TypedActionResult.success(itemStack, world.isClient());
    }

}
